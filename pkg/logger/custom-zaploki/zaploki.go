...
func newLog(entry logEntry) streamValue {
	...
	if err != nil {
		// Handle parsing error (e.g., fallback or log the error)
		ts = time.Now().UTC() // Default to current time if parsing fails
	}

	// Convert to Unix nanosecs for Loki
